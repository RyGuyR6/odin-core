...
BACKUP_DIR="$ROOT/.odin-backups/milestone21_5b_2/$STAMP"
...
step "Verifying an empty process variable masks backend/.env"
(
  cd "$BACKEND"
...
# settings may contain the backend/.env token, but the explicit empty
# process variable must take precedence in the runtime resolver.
assert settings.ODIN_GITHUB_TOKEN
client = GitHubClient()
assert client.configured is False
assert client.token is None
print("Explicit empty environment value masked backend/.env")
PY
)
ok "Backend dotenv masking passed"
...
printf '  • Dynamic ODIN_GITHUB_TOKEN resolution\n  • Explicit-empty masking of backend/.env during tests\n'
