...
printf '\n============================================================\n'
printf 'ODIN MILESTONE %s — DOTENV-AWARE GITHUB TEST ISOLATION\n' "$MILESTONE"
printf '============================================================\n'
...
BACKUP_DIR="$ROOT/.odin-backups/milestone21_5b_3/$STAMP"
...
step "Verifying an empty process variable forces an unconfigured client"
(
  cd "$BACKEND"
...
# The test does not assume whether Pydantic discovered backend/.env.
# It verifies only the required invariant: an explicitly empty process
# variable must force the runtime GitHub client to remain unconfigured.
assert "ODIN_GITHUB_TOKEN" in os.environ
assert os.environ["ODIN_GITHUB_TOKEN"] == ""

client = GitHubClient()
assert client.configured is False
assert client.token is None
assert client._session is None
print("Explicit empty environment value forced an unconfigured client")
PY
)
ok "Explicit-empty GitHub configuration passed"
...
printf '  • Dynamic ODIN_GITHUB_TOKEN resolution\n  • Explicit-empty runtime configuration during tests\n'
