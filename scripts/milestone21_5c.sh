...
printf '\n============================================================\n'
printf 'ODIN MILESTONE %s — STARTUP LIFECYCLE AND DIAGNOSTICS\n' "$MILESTONE"
printf '============================================================\n'
...
BACKUP_DIR="$ROOT/.odin-backups/milestone21_5c/$STAMP"
    ...
    def _required_service_failures(self) -> list[str]:
        ...
        for name, data in self.services.health().items():
            if not data.get("required"):
                continue
            if data.get("state") in {"error", "unconfigured", "stopped"}:
...
printf '  • Required and optional service readiness evaluation\n'
