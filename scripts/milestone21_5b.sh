...
printf '\n============================================================\n'
printf 'ODIN MILESTONE %s — GITHUB SERVICE CONSOLIDATION\n' "$MILESTONE"
printf '============================================================\n'
printf 'Repository: %s\nBackend:    %s\nPython:     %s\n' "$ROOT" "$BACKEND" "$PYTHON_BIN"
printf 'Mode: cumulative (includes the 21.5a lifecycle foundation)\n'
...
BACKUP_DIR="$ROOT/.odin-backups/milestone21_5b/$STAMP"
...
step "Installing cumulative lifecycle foundation"
...
step "Making the canonical GitHub client credential- and network-lazy"
...
PY
ok "Canonical GitHub client is now lazy"

step "Making GitHubProvider injectable and import-safe"
...
The exported ``github`` object is a compatibility proxy. Importing this
package does not create an authenticated client, open a network connection,
or require ODIN_GITHUB_TOKEN.
"""
...
PY
ok "Global GitHub provider replaced with lazy compatibility proxy"

step "Consolidating the legacy GitHubService onto the canonical client"
cat > "$BACKEND/app/services/github_service.py" <<'PY'
"""
Backward-compatible GitHub service facade.

New code should use ``app.services.github.GitHubProvider``. This facade keeps
older plugins and Odin's service container working while sharing the canonical
...
PY
ok "Legacy GitHub facade now shares the canonical client"

step "Registering the canonical provider lazily in Odin"
...
PY
ok "Odin now uses the canonical GitHub provider"

step "Adding FastAPI GitHub dependencies and error mapping"
...
def create_branch(
    ...
    return run(
        lambda: branches.create_branch(owner, repo, new_branch, source_sha)
    )
PY
...
PY
ok "GitHub API routes now use request-time dependencies"

step "Fixing shared dependency providers"
...
Coordinates GitHub domain services to complete higher-level engineering tasks.
Provider construction is safe without credentials; authentication is checked
only when an operation performs an HTTP request.
"""
...
PY
ok "Dependency providers now share one GitHub provider"

step "Adding regression tests"
...
def test_package_import_is_lazy_without_token(monkeypatch):
...
def test_fastapi_openapi_and_non_github_start_without_token(monkeypatch):
    monkeypatch.delenv("ODIN_GITHUB_TOKEN", raising=False)
    reset_github_provider()
...
def test_unconfigured_github_endpoint_returns_503(monkeypatch):
    monkeypatch.delenv("ODIN_GITHUB_TOKEN", raising=False)
    reset_github_provider()
    from app.main import app
...
PY
ok "Lifecycle and GitHub consolidation tests added"
...
step "Verifying package import without GitHub credentials"
(
  cd "$ROOT"
...
assert github.initialized is False
print("GitHub package imported without provider initialization")
PY
)
ok "GitHub package import is credential-free"

step "Verifying both GitHub entry points share canonical client"
(
  cd "$ROOT"
  ODIN_GITHUB_TOKEN="validation-token" PYTHONPATH="$BACKEND" "$PYTHON_BIN" - <<'PY'
from app.services.github import get_github_provider, reset_github_provider
from app.services.github.client import GitHubClient
from app.services.github_service import GitHubService

reset_github_provider()
provider = get_github_provider()
legacy = GitHubService()

assert isinstance(provider.client, GitHubClient)
assert isinstance(legacy.client, GitHubClient)
assert provider.client._session is None
assert legacy.client._session is None
print("Canonical client shared by provider and compatibility facade")
PY
)
ok "Duplicate client implementations consolidated"
...
step "Verifying unconfigured GitHub endpoint returns 503"
(
  cd "$ROOT"
  env -u ODIN_GITHUB_TOKEN PYTHONPATH="$BACKEND" "$PYTHON_BIN" - <<'PY'
from fastapi.testclient import TestClient
...
ok "GitHub API configuration error mapping passed"

step "Verifying configured construction performs no network request"
(
  cd "$ROOT"
  ODIN_GITHUB_TOKEN="validation-token" PYTHONPATH="$BACKEND" "$PYTHON_BIN" - <<'PY'
...
step "Checking consolidation invariants"
(
  cd "$ROOT"
...
legacy = Path("backend/app/services/github_service.py").read_text()
odin = Path("backend/app/core/odin.py").read_text()

assert "github = GitHubProvider()" not in package
assert "raise RuntimeError(\"ODIN_GITHUB_TOKEN not configured.\")" not in client
assert "from app.services.github.client import GitHubClient" in legacy
assert "get_github_provider" in odin
assert package.count("github = LazyGitHubProvider()") == 1
print("Consolidation invariants verified")
PY
)
ok "Consolidation invariants passed"
...
printf 'Backup:       %s\n' "$BACKUP_DIR"
printf '\nInstalled:\n'
printf '  • Cumulative 21.5a lazy service lifecycle foundation\n'
printf '  • One canonical GitHubClient implementation\n'
printf '  • Lazy global GitHub provider compatibility proxy\n'
printf '  • Legacy GitHubService compatibility facade\n'
printf '  • FastAPI request-time GitHub dependencies\n'
printf '  • HTTP 503 mapping for missing GitHub configuration\n'
printf '  • Credential-free FastAPI/OpenAPI startup\n'
printf '  • Consolidation and lifecycle regression tests\n'
