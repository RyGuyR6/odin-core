...
def test_planner_uses_repository_intelligence_for_metadata(
    ...
    plan = Planner().create_plan("Update the health API route", repository="acme/repo")
...
def test_planner_api_returns_repository_intelligence_context(
    ...
    response = client.post(
        "/planner/",
        json={"goal": "Update the health API route", "repository": "acme/repo"},
    )
    ...
    assert set(body["repository_summary"]["frameworks"]) == {
        "FastAPI",
        "Next.js",
        "Pydantic",
        "React",
