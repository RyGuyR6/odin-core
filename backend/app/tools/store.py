...
class ToolStore:
    ...
    def list_approvals(self, limit: int = 100, status: str | None = None):
    ...
    def audit_events(self, limit: int = 100):
        with self._connect() as connection:
            rows = connection.execute(
                "SELECT * FROM tool_audit_events ORDER BY id DESC LIMIT ?",
                (limit,),
            ).fetchall()
        return [dict(row) | {"payload": json.loads(row["payload_json"])} for row in rows]

    def execution_events(self, execution_id: str, limit: int = 100):
        with self._connect() as connection:
            rows = connection.execute(
                """
    ...
    def telemetry(self):
        with self._connect() as connection:
            rows = connection.execute(
                "SELECT status, COUNT(*) AS n FROM tool_executions GROUP BY status"
            ).fetchall()
        ...
        return counts, float(average or 0)
    ...
    @staticmethod
    def _execution_from_row(row) -> ToolExecutionRecord:
