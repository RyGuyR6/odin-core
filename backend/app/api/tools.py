...
def telemetry():
    ...
    counts, average = tool_manager.store.telemetry()
    return TelemetryResponse(
        ...
        tools_registered=len(tool_manager.registry.list()),
    )
...
def list_executions(
    ...
    rows = manager().store.list_executions(
        limit,
...
async def execute(request: ToolExecutionRequest):
    try:
        return await manager().executor.execute(request)
...
async def execute_approved(approval_id: str, context: ExecutionContext):
    ...
    try:
        return await manager().executor.resume_approved(approval_id, request)
    except ToolValidationError as exc:
        raise HTTPException(422, str(exc)) from exc
...
def permissions():
    ...
    permissions = [
        ...
        for definition in (tool.tool_definition() for tool in tool_manager.registry.all())
    ]
    return PermissionQueryResponse(
        shell_enabled=tool_manager.settings.allow_shell,
        python_enabled=tool_manager.settings.allow_python,
        require_approval_for_writes=tool_manager.settings.require_approval_for_writes,
        require_approval_for_shell=tool_manager.settings.require_approval_for_shell,
