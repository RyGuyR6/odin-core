...
def list_memories(limit:int=Query(100,ge=1,le=100),offset:int=Query(0,ge=0),scope:str|None=None,project_id:str|None=None,repository_id:str|None=None,kind:str|None=None,manager:MemoryManager=Depends(manager_dep)): return guard(lambda:manager.list(limit=limit,offset=offset,scope=scope,project_id=project_id,repository_id=repository_id,kind=kind))
...
def ingest_file(path:str,scope:str="global",project_id:str|None=None,repository_id:str|None=None,manager:MemoryManager=Depends(manager_dep)): return guard(lambda:manager.ingest_file(path,scope=scope,project_id=project_id,repository_id=repository_id))
