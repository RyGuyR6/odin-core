...
class RepositoryIndexer:
        ...
        for current, dirs, files in os.walk(root):
            ...
            for directory in sorted(dirs):
                if directory in IGNORE_DIRS:
                    ...
                    self._record_skip(
                        "ignored", Path(current).joinpath(directory).relative_to(root)
                    )
                    continue
    ...
    def manifest(
        self, workspace_id: str, root: Path, entries: list[FileIndexEntry]
    ) -> RepositoryManifest:
    ...
    def search_content(
        ...
        for path in iterator:
            if not path.is_file() or any(
                part in IGNORE_DIRS for part in path.relative_to(root).parts
            ):
                continue
            try:
